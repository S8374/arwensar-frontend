...
import { notificationApi } from "@/redux/features/notification/notification.api";
import {
  AlertCircle,
  CheckCircle2,
  AlertTriangle,
...
export const getActionFromNotification = (notification: Notification): {
  ...
  const { type, metadata } = notificationApi.endpoints.getNotifications.select()(undefined, undefined).data?.data.find(n => n.id === notification.id) || notification;

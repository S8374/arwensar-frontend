...
import {
  ...
  Building, CheckCircle2, XCircle, Clock
} from "lucide-react";
...
const getNis2Status = (supplier: any) => {
  const hasPending = supplier.statistics?.pendingSubmissions > 0 || !supplier.fullAssessmentCompleted;
  const isCompliant = supplier.nis2Compliant === true;

  if (!isCompliant) return { label: "Non-Compliant", variant: "outline" as const, icon: XCircle };
  if (hasPending) return { label: "Incomplete", variant: "destructive" as const, icon: Clock };
  return { label: "Compliant", variant: "outline" as const, icon: CheckCircle2 };
};
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Button variant="outline" onClick={() => setIsAlertOpen(true)}>
              <AlertCircle className="w-4 h-4 mr-2" />Send Alert
            </Button>
